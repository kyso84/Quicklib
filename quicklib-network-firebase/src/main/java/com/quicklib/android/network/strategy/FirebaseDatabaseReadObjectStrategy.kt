...
abstract class FirebaseDatabaseReadObjectStrategy<T>(private val databaseRef: DatabaseReference) : DataStrategy<T>() {
            ...
            try {
                ...
                databaseRef.addListenerForSingleValueEvent(object : ValueEventListener {
                    ...
                    override fun onCancelled(error: DatabaseError) {
                        liveData.postValue(DataWrapper(error = error.toException(), status = DataStatus.ERROR, localData = true, strategy = this@FirebaseDatabaseReadObjectStrategy::class))
                    }
                })
            } catch (error: Throwable) {
                liveData.postValue(DataWrapper(error = error, status = DataStatus.ERROR, localData = true, strategy = this@FirebaseDatabaseReadObjectStrategy::class))
            }
        } else {
            liveData.postValue(DataWrapper(error = IllegalStateException("Local value is not available"), status = DataStatus.ERROR, localData = true, strategy = this@FirebaseDatabaseReadObjectStrategy::class))
        }
    }
